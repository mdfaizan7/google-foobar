...
# sufficiently quick-witted, Commander Lambda has installed new flooring outside
# the henchman dormitories. It looks like a chessboard, and every morning and evening
# you have to solve a new movement puzzle in order to cross the floor.
# That would be fine if you got to be the rook or the queen, but instead,
...
# (that is, two squares in any direction immediately followed by one square perpendicular
# to that direction, or vice versa, in an "L" shape).
...
def solution(src, dest):
    [x1, y1] = getCoordinates(src)
    [x2, y2] = getCoordinates(dest)

    return moves[abs(x1 - x2)][abs(y1 - y2)]
