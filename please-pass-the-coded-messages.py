...
# with the numbers 0-9 for easier sorting, and you need to combine sets of plates to create the numbers in the code.
...
# Write a function solution(L) which finds the largest number that can be made from some or all of these digits
# and is divisible by 3. If it is not possible to make such a number, return 0 as the solution. 
...
def solution(l):
  ...
  for i in range(len(l), 0, -1):
    for comb in itertools.combinations(l, i):
      if sum(comb) % 3 == 0:
        return list2num(comb)

print(solution([3, 1, 4, 1, 5, 9]),solution([3, 1, 4, 1, 5, 9]) == 94311)
print(solution([3, 1, 4, 1]), solution([3, 1, 4, 1]) == 4311)